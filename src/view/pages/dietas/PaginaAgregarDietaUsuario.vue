        ...
        <v-row class="col-md-12">
          ...
          <v-col cols="12" sm="12" md="6">
            <v-btn
              ...
              @click="modalNuevo"
            >
              <v-icon left>fas fa-user-plus</v-icon> Agregar Dieta
            </v-btn>
          </v-col>
        </v-row>
          ...
          >
            <v-row>
              <v-col cols="12" md="12" sm="12" class="pt-4 pb-0">
                <v-autocomplete
                :loading="dietasLoading"
                  v-model="datosItem.dietasId"
                  :items="dietas"
                  class="required"
                  dense
                  filled
...
import { OBTENER_DIETAS } from "@/core/services/store/dietas/dietas.module";
import {
  OBTENER_DIETAS_USUARIO,
  OBTENER_DIETA_USUARIO,
  REGISTRAR_DIETA_USUARIO,
  ACTUALIZAR_DIETA_USUARIO,
} from "@/core/services/store/dietas/dietasusuarios.module";
...
export default {
  ...
  data() {
    return {
      ...
      filtro: "",
      dialog: false,
      modalTitle: "Agregar nueva dieta.",
      accion: 1,
  ...
  methods: {
    ...
    async obtenerItems(Id) {
      ...
      this.$store
        .dispatch(OBTENER_DIETAS_USUARIO, Id)
        .then((res) => {
          if (res.status === 200) {
            this.items = this.$store.state.dietasusuarios.dietasUsuario;
    ...
    /// Obtener los items
    async obtenerItemsDietas() {
      this.dietas = [];
      this.dietasLoading = true;

      this.$store
        .dispatch(OBTENER_DIETAS, 1)
        .then((res) => {
          if (res.status === 200) {
            this.dietas = this.$store.state.dietas.dietas;
            //console.log(this.items);
          }
          this.dietasLoading = false;
          //console.log(this.items);
          //console.log('sdkfdkd');
        })
        .catch(() => {
    ...
    //     : (this.datosItem.dia = "Domingo");
      ...
      if (this.accion === 1) {
        ...
        this.$store
          .dispatch(REGISTRAR_DIETA_USUARIO, this.datosItem)
          .then((res) => {
            console.log();
      ...
      //?Si accion es 2 se realiza la actualizacion
        ...
        //console.log(this.datosItem);
        this.$store
          .dispatch(ACTUALIZAR_DIETA_USUARIO, this.datosItem)
          .then((res) => {
            //console.log(res);
            this.dialogLoaderVisible = false;
            this.btnRegistroLoading = false;
    ...
    obtenerDatosItem(Id) {
      ...
      this.$store
        .dispatch(OBTENER_DIETA_USUARIO, Id)
        .then((res) => {
          if (res.status === 200) {
            this.datosItem = this.$store.state.dietasusuarios.dietaUsuario;
            console.log(this.datosItem);
            this.datosItem.estadosId === 1
              ? (this.switchItemEstado = true)
