            ...
            <v-row>
              <v-col cols="12" md="12" class="pt-4 pb-0">
                <v-text-field
                  ...
                  filled
                  color="blue-grey darken-2"
                  v-model="datosItem.departamento"
                  label="Departamento"
                  :rules="[
                    required('Departamento'),
                    minLength('Departamento', 3),
...
import { OBTENER_DEPARTAMENTOS,REGISTRAR_DEPARTAMENTO,ACTUALIZAR_DEPARTAMENTO,OBTENER_DEPARTAMENTO } from "@/core/services/store/departamentos/departamentos.module";
...
export default {
  ...
  data() {
    return {
      ...
      filtro: "",
      dialog: false,
      modalTitle: "Registrar nuevo departamento",
      accion: 1,
  ...
  methods: {
    ...
    modalNuevo() {
      this.accion = 1;
      this.modalTitle = "Registrar Departamento";
      this.btnRegistroText = "Registrar Departamento";
    ...
    this.items = [];
      ...
      this.$store
        ...
        .then(res => {
          if(res.status === 200){
            this.items = this.$store.state.departamentos.departamentos;
          }
    ...
    registrarItem() {
      ...
      if (this.accion === 1) {
        this.$store
          .dispatch(REGISTRAR_DEPARTAMENTO, this.datosItem)
          .then(res => {
      ...
      //?Si accion es 2 se realiza la actualizacion
        ...
        this.$store
          .dispatch(ACTUALIZAR_DEPARTAMENTO,  this.datosItem)
          .then(res => {
            this.btnLoading = false;
            this.dialogLoaderVisible = false;
            this.btnRegistroLoading = false;
    ...
    obtenerDatosItem(Id) {
      ...
      this.$store
        .dispatch(OBTENER_DEPARTAMENTO, Id)
        .then(res => {
          //console.log(res);
          if (res.status === 200) {
            this.datosItem = this.$store.state.departamentos.departamento;
            this.datosItem.estadosId === 1
              ? (this.switchItemEstado = true)
