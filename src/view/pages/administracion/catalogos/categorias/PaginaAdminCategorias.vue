...
import { OBTENER_CATEGORIAS,REGISTAR_CATEGORIA,ACTUALIZAR_CATEGORIA,OBTENER_CATEGORIA } from "@/core/services/store/categorias/categorias.module";
...
export default {
  ...
  methods: {
    ...
    this.items = [];
      ...
      this.$store
        .dispatch(OBTENER_CATEGORIAS)
        .then(res => {
          if(res.status === 200){
            this.items = this.$store.state.categorias.categorias;
          }
    ...
    registrarItem() {
      ...
      if (this.accion === 1) {
        this.$store
          .dispatch(REGISTAR_CATEGORIA, this.datosItem)
          .then(res => {
      ...
      //?Si accion es 2 se realiza la actualizacion
        ...
        this.$store
          .dispatch(ACTUALIZAR_CATEGORIA,  this.datosItem)
          .then(res => {
            this.btnLoading = false;
            this.dialogLoaderVisible = false;
            this.btnRegistroLoading = false;
